...
def train_func(args: dict, data_config_obj: dict, model_config_obj: dict):
    ...
    # wandb.config['githash'] = args['githash']
    # wandb.config['model_structure'] = repr(model)
...
# import argparse
# parser = argparse.ArgumentParser(prog='trainloop')
...
for lmmodel_config, optimer_config in itertools.product(model_class_list, optim_class_list):
    ...
    data_config = {
        "train_pos_seq_list": list(
            SeqIO.parse(
                'data/T3_pos_filter.fasta', 'fasta'
            )
        )[0:358],
        "train_neg_seq_list": list(
            SeqIO.parse(
                'data/T3_neg_filter.fasta', 'fasta'
            )
        )[0:12915],
